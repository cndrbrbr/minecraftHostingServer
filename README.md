...
Or start servers individually using the generated scripts:

```bash
./start-mc1.sh   # starts / restarts mc1 only
./start-mc3.sh   # starts / restarts mc3 only
```
...
### Start / restart a single server

```bash
./start-mc3.sh        # (re)start mc3 — generated by configure-memory.sh
```

This calls `docker compose up -d --no-deps mc3`, which recreates the container if configuration (e.g. memory) changed, or just starts it if it was stopped.

### Restart only the Minecraft process (keep container running)

```bash
./restart.sh 3        # sends SIGTERM to the Java process inside mc3
```
